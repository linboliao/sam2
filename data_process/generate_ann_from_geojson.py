import json
import os
import re

import cv2
import numpy as np
from PIL import Image
from loguru import logger
from matplotlib import pyplot as plt
from pycocotools import mask as mask_utils


def get_mask(self, image_path):
    lower_bound = np.array([20, 20, 30])
    upper_bound = np.array([130, 130, 160])

    image = cv2.imread(image_path)
    image = cv2.copyMakeBorder(image, 3, 3, 3, 3, cv2.BORDER_CONSTANT, value=(0, 0, 0))

    # 分离暗区和亮区
    return cv2.inRange(image, lower_bound, upper_bound)


geojson_path = r'/data2/yhhu/LLB/Data/前列腺癌数据/DAB染色/qpdata/mx1631040.2-HE.geojson'
with open(geojson_path, 'r') as f:
    geo = json.load(f)

features = geo.get('features', [])

folder = r'/data2/yhhu/LLB/Data/前列腺癌数据/DAB染色/patch/4096/image/'
ann_folder = r'/data2/yhhu/LLB/Data/前列腺癌数据/DAB染色/patch/4096/seg_train/annotation/'
img_folder = r'/data2/yhhu/LLB/Data/前列腺癌数据/DAB染色/patch/4096/seg_train/image/'
file_names = os.listdir(folder)
for fn in file_names:
    if 'mx1631040.' not in fn or 'CKpan' in fn:
        continue
    objects = []
    match = re.search(r'_(\d+)_(\d+)\.jpg', fn)
    x_str, y_str = match.groups()
    x, y = int(x_str), int(y_str)

    if x == 16384 and y == 16384:
        img = cv2.imread(os.path.join(folder, fn))
        for feature in features:
            if feature.get('geometry', {}).get('type') == "Polygon":

                coords = feature.get('geometry', {}).get('coordinates', [])
                mark = coords[0][0]

                if x < mark[0] < x + 4096 and y < mark[1] < y + 4096:
                    idx = 1 if feature.get("properties", {}).get("classification") is not None else 0
                    color = (0,255,0) if feature.get("properties", {}).get("classification") is not None else (0,0,255)

                    for coord in coords:
                        coord_points = [[(a - x), (b - y) ] for a, b in coord]
                        cnt = np.array(coord_points, dtype=np.int32).reshape(-1, 1, 2)
                        image = np.zeros((1024, 1024), dtype=np.uint8)

                        contours = [cnt]


                        cv2.drawContours(img, contours, -1, color, 2)
                        cv2.drawContours(image, contours, -1, 255, 2)
                        rle = mask_utils.encode(np.array(image[:, :, None], order='F', dtype="uint8"))[0]
                        rle["counts"] = rle["counts"].decode("utf-8")

                        rect = cv2.minAreaRect(cnt)
                        (center_x, center_y), (width, height) = rect[0], rect[1]

                        obj = {
                            "id": idx,  # Annotation id
                            "category_id": idx,
                            "segmentation": rle,  # Mask saved in COCO RLE format.
                            "bbox": [cv2.boundingRect(cnt)],  # The box around the mask, in XYWH format
                            "area": int(cv2.contourArea(cnt)),  # The area in pixels of the mask
                            "predicted_iou": 1,  # The model's own prediction of the mask's quality
                            "stability_score": 1,  # A measure of the mask's quality
                            "crop_box": [int(center_x - width / 2), int(center_y - height / 2), width, height],
                            "point_coords": coord_points,  # The point coordinates input to the model to generate the mask
                        }
                        # if cancer_flag:
                        # else:
                        objects.append(obj)
        # else:
        #     multi_coords = feature.get('geometry', {}).get('coordinates', [])
        #     for coords in multi_coords:
        #         mark = coords[0][0]
        #         if x < mark[0] < x + 4096 and y < mark[1] < y + 4096:
        #             cancer_flag = feature.get("properties", {}).get("classification") is not None
        #             for coord in coords:
        #                 if cancer_flag:
        #                     obj = {
        #                         "category": "non-cancer",
        #                         "group": 0,
        #                         "segmentation": coords
        #                     }
        #                 else:
        #                     obj = {
        #                         "category": "cancer",
        #                         "group": 1,
        #                         "segmentation": coords
        #                     }
        #                 objects.append(obj)
        img_rgb = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)

        # 使用matplotlib显示图像
        plt.imshow(img_rgb)
        plt.axis('off')  # 不显示坐标轴
        plt.show()
    # if len(objects) == 0:
    #     continue
    # annotation = {
    #     "image": {
    #         "image_id": "",
    #         "folder": r"/data2/yhhu/LLB/Data/前列腺癌数据/DAB染色/patch/4096/image/",
    #         "file_name": fn,
    #         "width": 4096,
    #         "height": 4096,
    #     },
    #     "annotations": objects
    # }
    # os.makedirs(ann_folder, exist_ok=True)
    # os.makedirs(img_folder, exist_ok=True)
    # bn = os.path.splitext(fn)[0]
    # with open(os.path.join(ann_folder, f'mx1631040_{x}_{y}.json'), 'w') as f:
    #     json.dump(annotation, f, indent=2)
    # logger.info(f'process {fn}')
    # img = Image.open(os.path.join(folder, fn))
    # resized_img = img.resize((1024, 1024), Image.Resampling.LANCZOS)
    # resized_img.save(os.path.join(img_folder, f'mx1631040_{x}_{y}.jpg'))

